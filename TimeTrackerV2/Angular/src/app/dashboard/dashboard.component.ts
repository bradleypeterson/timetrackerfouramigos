...
export class DashboardComponent implements OnInit {
  ...
  getUser(): void {
    ...
    this.httpService.getCookie().subscribe((_users: any) => {
      ...
      this.httpService
        .getUserCourses(this.user.userID)
        .subscribe((_courses: any) => {
          this.courses = _courses;
          this.cSize = this.courses.length;
  ...
  getUserGroups(id: number) {
    this.httpService.getUserGroups(id).subscribe((_groups: any) => {
      this.groups = _groups;
      this.gSize = this.groups.length;
    });
  }

  // get the user's groups
  getUserProjects(id: number) {
    this.httpService.getUserProjects(id).subscribe((_projects: any) => {
      this.projects = _projects;
      this.pSize = this.projects.length;
    });
  }
